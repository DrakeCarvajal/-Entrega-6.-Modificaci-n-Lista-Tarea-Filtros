...
class TarjetaTarea extends StatelessWidget {
  final Tarea tarea; // Objeto que representa la tarea a mostrar
  final int indice; // Índice de la tarea en la lista

  const TarjetaTarea({super.key, required this.tarea, required this.indice});

  @override
  Widget build(BuildContext context) {
    // Accedemos al proveedor de tareas para manejar las acciones relacionadas con las tareas
    ...
    return Card(
      ...
      child: ListTile(
        // Checkbox animado para marcar o desmarcar la tarea como completada
        leading: AnimatedSwitcher(
          ...
          child: Checkbox(
            ...
            ), // Clave única para animación
            value: tarea.estaCompletada, // Estado actual de la tarea
            onChanged: (value) =>
                tareaProvider.cambiarEstadoTarea(indice), // Cambiar estado
            activeColor: Theme.of(
              context,
            ).colorScheme.secondary, // Color del checkbox activo
            checkColor: Theme.of(
              context,
            ).colorScheme.onSecondary, // Color del check
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(
                4,
              ), // Bordes redondeados del checkbox
            ),
          ),
        ),
            ...
            Icon(
              ...
              color: tarea.estaCompletada
                  ? Theme.of(context).colorScheme.onSurface.withAlpha(
                      128,
                    ) // Color atenuado si está completada
                  : Theme.of(context)
                        .colorScheme
                        .onSurface, // Color normal si no está completada
              size: 20, // Tamaño del ícono
            ),
            const SizedBox(width: 8), // Espaciado entre ícono y texto
            Expanded(
              child: Text(
                "${tarea.titulo} - ${tarea.provincia}", // Título de la tarea
                style: TextStyle(
                  decoration: tarea.estaCompletada
                      ? TextDecoration.lineThrough
                      : null, // Línea tachada si está completada
                  color: tarea.estaCompletada
                      ? Theme.of(context).colorScheme.onSurface.withAlpha(
                          128,
                        ) // Color atenuado si está completada
            ...
            ? Row(
                children: [
                  Icon(
                    ...
                    color: Theme.of(
                      context,
                    ).colorScheme.onSurface.withAlpha(128), // Color atenuado
                    size: 20, // Tamaño del ícono
                  ),
                  const SizedBox(width: 8), // Espaciado entre ícono y texto
                  Expanded(
        ...
        trailing: IconButton(
          ...
          onPressed: () => tareaProvider.eliminarTarea(
            indice,
