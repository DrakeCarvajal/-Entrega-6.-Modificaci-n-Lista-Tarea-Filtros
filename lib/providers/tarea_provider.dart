...
class TareaProvider with ChangeNotifier {
  ...
  // Filtro por provincia, por defecto 'Todas'
  ...
  List<Tarea> get tareasFiltradas {
    ...
    if (_filtroCategoria != 'Todas') {
      tareasFiltradas = tareasFiltradas
          .where((tarea) => tarea.categoria == _filtroCategoria)
          .toList();
    }
    ...
    if (_filtroEstado != 'Todas') {
      if (_filtroEstado == 'Completadas') {
        tareasFiltradas = tareasFiltradas
            .where((tarea) => tarea.estaCompletada)
            .toList();
    ...
    if (_filtroProvincia != 'Todas') {
      tareasFiltradas = tareasFiltradas
          .where((tarea) => tarea.provincia == _filtroProvincia)
          .toList();
    }
  ...
  void cambiarEstadoTarea(int indice) {
    _tareas[indice].estaCompletada =
        !_tareas[indice].estaCompletada; // Alternar el estado
    guardarDatos(); // Guardar los cambios en SharedPreferences
    notifyListeners(); // Notificar a los widgets que dependen de este proveedor
  }
  ...
  void cambiarFiltroProvincia(String provincia) {
    _filtroProvincia = provincia;
    notifyListeners();
  }
}
